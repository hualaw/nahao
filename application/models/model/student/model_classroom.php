...
class Model_Classroom extends NH_Model{
    ...
    {
        $array_result = array();
        $sql = "SELECT MAX(sequence) AS sequence FROM ".TABLE_QUESTION_CLASS_RELATION." WHERE class_id = ".$int_class_id;
    ...
    {
        $array_result = array();
        $sql = "SELECT question_id FROM ".TABLE_QUESTION_CLASS_RELATION." WHERE class_id = ".$int_class_id." AND status = 1 AND sequence = ".$int_max_sequence;
    ...
    {
        $array_result = array();
        $sql ="SELECT question_id,answer AS selected,sequence,is_correct FROM ".TABLE_STUDENT_QUESTION." WHERE class_id = ".$array_data['class_id']." AND student_id = ".$array_data['student_id']." AND sequence = ".$array_data['sequence'];
        $array_result = $this->db->query($sql)->result_array();
        return $array_result;
    }
    ...
    {
        $sql = "SELECT id FROM ".TABLE_QUESTION_CLASS_RELATION." WHERE class_id = ".$array_data['class_id']." AND status = 1 AND sequence = ".$array_data['sequence'];
    ...
    {
        $sql ="SELECT id FROM ".TABLE_STUDENT_QUESTION." WHERE class_id = ".$array_data['class_id']." AND student_id = ".$array_data['student_id']." AND sequence = ".$array_data['sequence']." AND is_correct = ".$is_correct;
        $int_result = $this->db->query($sql)->num_rows();
        return $int_result;
    }
    ...
    {
        $array_result = array();
        $sql = "SELECT distinct question_id FROM ".TABLE_STUDENT_QUESTION." WHERE class_id = ".$int_class_id." AND sequence = " .$int_max_sequence." AND student_id = ".$int_user_id;
        $array_result = $this->db->query($sql)->result_array();
        return $array_result;
    }
